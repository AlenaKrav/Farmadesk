...
class Tarea{
    ...
    public function __set($nombre, $valor) {
        if ($nombre == 'nombre_user_actualiza') {
            $this->nombre_user_actualiza = $valor;  // Si se intenta acceder a 'role_nombre', la asignamos
        }
    }

    public static function crear($nombre, $descripcion){
        $conexion = BD::crearInstancia();
        //no aparece fecha_creacion en la bd es por defecrto es current timestamp

        $sql = $conexion->prepare("INSERT INTO `tbl_tareas_farma`(`id`, `nombre`, `descripcion`, `estado`, `fecha_creacion`, `user_actualiza`, `fecha_actualiza`) VALUES (NULL, :nombre, :descripcion, 'Pendiente', CURRENT_TIMESTAMP, NULL, NULL);");
